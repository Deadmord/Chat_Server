...
void Server::incomingConnection(qintptr socket_descriptor_)
{
    ...
    connect(user_connection, &UserConnection::jsonReceived, MessageController::instance(this), std::bind(&MessageController::jsonReceived, MessageController::instance(this), user_connection, std::placeholders::_1));  //connect with MessageController (это нужно перенести в MessageController)
